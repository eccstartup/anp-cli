...
flowchart TD
    ...
    subgraph L1["命令层 internal/cli（只做参数解析 + 渲染，不写业务）"]
        ...
        Handlers["Handler 分发表<br/>id / msg / group / discovery / proof / e2ee / runtime / daemon"]
    ...
    subgraph L2["业务层（不 fmt.Println）"]
        ...
        SGrp["group<br/>create / join / leave / members"]
    ...
    subgraph L3["协议 + 存储层"]
        ...
        Store["store<br/>SQLite：消息 / 联系人 / 群 / 发现索引"]
        Doc["doctor<br/>环境诊断"]
    end
...
sequenceDiagram
    ...
    TP->>BE: POST /rpc  msg.send  {to, body:{text}}
    BE-->>TP: {message_id, thread_id, sent_at}
